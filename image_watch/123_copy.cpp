    ...
    if(task_update == true)//需要更新
    {
        if(ready_flag == true)//已经完成了，可以开始了下一轮的处理
        {
            ...
            if(dmax <= 1)
                ...
                cvConvert(Image_all.pTemp,Image_all.pTemp_8U);
//          cvGetROI(this->pTemp_8U,this->pShow_8U,&CutOutRect);
...
//                    cvReleaseImage(&pDstImage);
    ...
    if( mouse_move_statue == true)
    {
        ...
        QtUpdateStatusBar(Image_all.pShow,&this->CutOutRect,mouse_locate);
    }
}
...
******************************/
void MainWindow::cvGetSubImage(CvMat *pSrc,CvMat *pDst,QRectF *CutOutRect)
{
...
{
//    *Gray = QString::number(gray, 10);
    *Gray = QString::number(static_cast<double>(gray), 'f', 2);
}
...
*********************************************/
template <class T>
void MainWindow::cvGetPixel(CvMat *img,int x, int y,T *r,T *g, T *b)
{
...
*********************************************/
void MainWindow::QtSetSquare_Indexed8(CvMat *img, QPainter *painter,int x, int y, float width, float height, QPen *pen)
{
    int r;
    int g;
    int b;
 ...
 * pRgb                         ：彩色图像
*********************************************/
void MainWindow::cvGray2Rgb(CvMat *pGray, CvMat *pRgb)
{
    int gray_cn = CV_MAT_CN(pGray->type);
    int gray_width = pGray->width;
    int gray_height = pGray->height;
